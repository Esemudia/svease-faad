...
namespace saveasze
{
    public partial class ReleaseNoteAdd_old : System.Web.UI.Page
    {
        ...
        public void SearchPO1()
        {
           ...
           // dt = dtproc.getApprovedPO("", "A", "", out expMess);
            //dt = dtproc.getAllATLPO("", "ATLC", "", out expMess);
            dt = dtproc.getAllRelNotePO("", "RELC", "", out expMess);
            drppo.DataSource = dt;
        ...
        protected void btnSubmit_ServerClick(object sender, EventArgs e)
        {
            ...
            if (ctrATL > 0 && strAtlID != "")
            {
                ...
                lblMessage.Text = "PO " + strPo + " is created or already exist";
        ...
        protected void drppo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string expMess = "";
            //getPOLike(string po, string connLocation, out string expMessage)
            dt = dtproc.getAllRelNotePO(drppo.SelectedValue, "RELC", "", out expMess);
           // dt = dtproc.getATByPO(drppo.SelectedValue, "", out expMess);
            foreach (DataRow dr in dt.Rows)
            {
                ...
                prdType.Text = dr["product_ID"].ToString();
                prdTypeID.Value = dr["productName"].ToString();
                lblQty.Text = dr["qty"].ToString();
                lblPrice.Text = dr["uprice"].ToString();
                double prdAmt = double.Parse(dr["Qty"].ToString()) * double.Parse(dr["uprice"].ToString());
                lblTotPrdAmount.Text = prdAmt.ToString();
                
                string strVat = dr["vat"].ToString();
                string strwitholding = dr["witholding"].ToString();
                string strncd = dr["NCD"].ToString();
                double dblSubTotal = prdAmt;
